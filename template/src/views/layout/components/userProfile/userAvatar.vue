...
import { uploadAvatar } from "@/api/framework/user";
...
export default {
  ...
  data() {
    return {
      ...
      title: "修改头像",
      options: {
        img: this.getAvatar(store.getters.avatar), //裁剪图片的地址
  ...
  methods: {
    // 获取头像地址
    getAvatar(avatar) {
      // 判断是否有自定义头像，没有则使用默认头像
      if (avatar.toString().indexOf('.jpg')!==-1) {
        return avatar;
      } else {
    ...
    uploadImg() {
      this.$refs.cropper.getCropBlob(data => {
        ...
        uploadAvatar(formData).then(response => {
          ...
          // 设置头像链接
          this.getAvatar(response.fileId);
          store.commit('SET_AVATAR', response.fileId);
